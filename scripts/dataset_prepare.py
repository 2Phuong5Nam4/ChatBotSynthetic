...
def clean_messages(messages: list[dict]) -> list[dict]:
    ...
    for message in messages:
        ...
        if '<think>' in content and '</think>' in content:
            ...
            content = (content[:content.index('<think>')] + content[end_idx + len('</think>'):]).strip()
        ...
        if '<tool_call>' in content and '</tool_call>' in content:
            ...
            for match in matches:
                ...
                # Parse tool call: could be "tool_name({...})" or '{"name": "...", "arguments": {...}}'
                
                # Try format: tool_name({...}) or tool_name({...})
                func_pattern = r'^(\w+)\s*\(\s*(\{.*\})\s*\)$'
                func_match = re.match(func_pattern, tool_call_content, re.DOTALL)
                ...
                if func_match:
                    ...
                    try:
                        ...
                        arguments = args_str  # Keep as string if not valid JSON
                    tool_calls.append({
                        "name": tool_name,
                        "arguments": arguments
                    })
